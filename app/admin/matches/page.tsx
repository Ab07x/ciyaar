    ...
    return (
        <div className="space-y-8">
            <div className="flex items-center justify-between">
                ...
                <Link href="/admin/matches/new" className="px-4 py-2 bg-accent-green text-black rounded-lg font-bold flex items-center gap-2"><Plus size={18} />Add Match</Link>
            </div>
                    ...
                    <tbody>
                        {filtered?.map(m => (
                            <tr key={m._id} className="border-b border-border-subtle last:border-0">
                                ...
                                <td className="px-4 py-3"><div className="flex items-center justify-end gap-2"><Link href={`/admin/matches/${m._id}`} className="p-2 hover:bg-stadium-hover rounded-lg"><Edit size={16} /></Link><button onClick={() => deleteMatch({ id: m._id })} className="p-2 hover:bg-stadium-hover rounded-lg text-accent-red"><Trash2 size={16} /></button></div></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
