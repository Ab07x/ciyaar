...
export default defineSchema({
    ...
    ppv_content: defineTable({
        ...
        contentId: v.string(), // slug or match ID
        title: v.string(),
        price: v.number(), // e.g., 1.99 USD
        adSupportedEnabled: v.boolean(), // Allow free with ads
        minAdsRequired: v.number(), // Ads before unlock (default 3)
        accessDuration: v.number(), // Hours of access (e.g., 24, 48)
        isActive: v.boolean(),
        createdAt: v.number(),
        updatedAt: v.number(),
    })
    ...
    ppv_purchases: defineTable({
        ...
        price: v.number(),
        accessType: v.union(
            v.literal("paid"),        // Full access, no ads
            v.literal("ad_supported") // Free with ads
        ),
        adsWatched: v.number(), // Track ads watched for ad-supported
        expiresAt: v.number(), // Unix timestamp when access expires
        createdAt: v.number(),
    })
        .index("by_user", ["userId"])
        .index("by_content", ["contentType", "contentId"])
    ...
    ratings: defineTable({
        ...
        contentId: v.string(), // slug
        rating: v.number(), // 1-5 stars
        review: v.optional(v.string()), // Max 500 chars
        isVerifiedWatch: v.boolean(), // User actually watched it
        helpfulCount: v.number(),
        reportCount: v.number(),
        isHidden: v.boolean(), // Hidden by admin/spam filter
        createdAt: v.number(),
        updatedAt: v.number(),
    })
        .index("by_content", ["contentType", "contentId"])
